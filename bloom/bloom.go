...
func (f *BloomFilter) Hashes(data string) []uint {
	h := baseHashes([]byte(data))
	hashes := make([]uint, f.k)
	for i := uint(0); i < f.k; i++ {
		hashes[i] = f.location(h, i)
	}
	return hashes
}
...
func main() {
	...
	// f := NewWithEstimates(1000000, 0.01) // load of 20, 5 keys
	// // filter.Add([]byte("Love"))
	// hashes := f.Hashes([]byte("Love"))
	// // h := baseHashes([]byte("data"))
